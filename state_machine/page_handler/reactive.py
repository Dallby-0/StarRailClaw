...
def controller_from_strategies(strategies: Any, *, exploration: Any = None) -> dict[str, Any] | None:
    """Best-effort migration for persisted v1 handlers.

    Strategies are flattened into independent providers. Their original order
    remains encoded in cost, while the reactive cursor prevents an already
    useful first step from being replayed after the screen changes.
    """
    ...
    for strategy_index, strategy in enumerate(strategies, start=1):
        if not isinstance(strategy, dict) or str(strategy.get("status", "proposed")) not in {"proposed", "active"}:
            continue
        level = max(0, int(strategy.get("level", 0) or 0))
        strategy_id = _slug(strategy.get("strategy_id"), f"strategy_{strategy_index}")
        for step_index, step in enumerate(strategy.get("steps") if isinstance(strategy.get("steps"), list) else [], start=1):
            if not isinstance(step, dict) or not isinstance(step.get("resolver"), dict):
                continue
            provider_index += 1
            provider = normalize_provider(
                {
                    "provider_id": f"{strategy_id}_{step.get('step_id') or f'step_{step_index}'}",
                    "kind": "action",
                    "cost": level * 10 + step_index - 1,
                    "repeat_policy": "once_per_visit",
                ...
                default_cost=level * 10 + step_index - 1,
            )
            if provider is not None:
                providers.append(provider)
    controller: dict[str, Any] | None = None
    if not providers:
        # Exploration-only migration remains useful when every exact strategy
    ...
    if isinstance(exploration, list) and exploration:
        fallback = normalize_provider(
            {
                "provider_id": "local_exploration",
                "kind": "exploration",
        ...
        if fallback is not None:
            if controller is None:
                controller = {"type": REACTIVE_CONTROLLER_TYPE, "providers": [], "max_actions": 12, "max_observation_rounds": 6, "created_at": _now_iso(), "updated_at": _now_iso()}
            controller["providers"].append(fallback)
    return controller
